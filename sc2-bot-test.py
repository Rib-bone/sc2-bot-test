...
class ThreebaseVoidrayBot(sc2.BotAI):
    ...
    async def defensive_patrol(self):
        location = random.choice(self.units(UnitTypeId.NEXUS)).position.to2 + self.get_random_pos()
        for unit in self.defencive_force.idle:
            await self.do(unit.attack(location))

    async def on_step(self, iteration):
        if iteration == 0:
            self.defencive_force: Units = Units([])
            self.attack_force: Units = Units([])
        ...
        if self.workers.amount < self.units(UnitTypeId.NEXUS).amount*15 and nexus.is_idle:
        ...
        for stargate in self.units(UnitTypeId.STARGATE).ready.idle:
